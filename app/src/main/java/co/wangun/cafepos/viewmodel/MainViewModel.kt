...
class MainViewModel: ViewModel() {
    ...
    fun invoiceInReceipt(invoice: String): String {
        val static = "Customer Number"
        val totalLen = 40
        val staticLen = static.length
        val invLen = invoice.length
        val space = " ".repeat(totalLen - staticLen - invLen)
        return "$static$space$invoice"
    }

    fun headerInReceipt(num: Int, dateTime: String): String {
        val totalLength = 40
        val table = "Table $num"
    ...
    fun totalInReceipt(total: Double): String {
        val static = "Total"
        val price = fu.toLocale(total, true)
    ...
    fun itemInReceipt(nameInput: String, amountInput: Int, priceInput: Double): String {
        var name = nameInput
    ...
    fun deleteItem(id: Long, table: String, name: String = "") {
        ...
        when(table) {
            ...
            TABLE_INVENTORY -> dbInventory.transactionWithResult {
                dbInventory.delete(id); null
            }
            else -> null
        }
    }
